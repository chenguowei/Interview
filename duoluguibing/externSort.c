...
void k_num_read(void)
{
  ...
  //计算 knum, 每路应该读取的整数个数 int n = NUMBER / KNUM
  int n = 0;
  if ( n * KNUM < NUMBER)
    n++;
...
void k_num_merge(void)
{
  ...
  // 建立 KNUM 个元素的 data, hasNext数组，存储k路文件的临时数组和读取结束状态
  data = (int*)malloc (KNUM* sizeof (int));
  assert (data);
  hasNext = (char *) malloc (KNUM * sizeof (char));
  assert (hasNext);
  memset (data, 0, sizeof (int) * KNUM);
  memset (hasNext, 1, sizeof (char) * KNUM);


  // 读 k 路文件先读取第一组数据，并对读取结束的各路文件设置不可再读状态
  for (i = 0; i < KNUM; i++)
    {
