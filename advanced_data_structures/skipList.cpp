...
void DebugOutput(const string& infomation)
{
#ifdef Debug
...
{
  Node update[MAX_LEVEL]; // 记录所有比 key大的结点
  int i;
  Node x = list_->header;
  // 寻找key 所要插入的位置
  // 保存大约 key 的位置信息
  ...
  if (x->key == key)
  ...
  else
    {
      ...
      // 调整 forward指针
      for (int j = level; i >= 0; ++j)
        {
          x = update[i];
          newNode->forward[i] = x->forward[i]; // 指向下一个元素
          x->forward[i] = newNode;
        }
...
{
  int level = static_cast<int>(rnd_.UniFrom (MAX_LEVEL));
  if (level == 0)
    {
      level = 1;
    }
